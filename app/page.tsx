...
import { Plus, RefreshCw, Activity, Globe, Wifi, TrendingUp } from "lucide-react"
...
function DashboardContent() {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: `${data.name} has been ${editingWebsite ? "updated" : "added"} successfully.`,
        })
        setShowForm(false)
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: `${website.name} monitoring has been ${isActive ? "enabled" : "paused"}.`,
        })
        fetchWebsites()
      } else {
  ...
  const handleCheckNow = async (website: Website) => {
    try {
      const response = await fetch("/api/monitor/single", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ websiteId: website.id, url: website.url, type: website.type }),
      })
  ...
  const getOverallStats = () => {
    ...
    const up = websites.filter((w) => w.statusChecks.length > 0 && w.statusChecks[0].status === "up").length
    const down = websites.filter((w) => w.statusChecks.length > 0 && w.statusChecks[0].status === "down").length
    const avgResponseTime =
      websites
        .filter((w) => w.statusChecks.length > 0 && w.statusChecks[0].responseTime)
        .reduce((acc, w) => acc + (w.statusChecks[0].responseTime || 0), 0) /
        websites.filter((w) => w.statusChecks.length > 0 && w.statusChecks[0].responseTime).length || 0

    return { total, active, up, down, avgResponseTime: Math.round(avgResponseTime) }
  }
      ...
      <div className="container mx-auto p-6 space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600">Monitor your websites and servers in real-time</p>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" onClick={handleMonitorAll} disabled={monitoring} className="h-10 bg-transparent">
              <RefreshCw className={`h-4 w-4 mr-2 ${monitoring ? "animate-spin" : ""}`} />
              {monitoring ? "Checking..." : "Check All"}
            </Button>
            <Button onClick={() => setShowForm(true)} className="h-10 bg-blue-600 hover:bg-blue-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="border-0 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-slate-600">Total Monitors</CardTitle>
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">Services down</p>
            </CardContent>
          </Card>
